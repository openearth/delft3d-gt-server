...
from delft3dworker.views import ScenarioViewSet, SceneViewSet, UserViewSet
...
class ApiAccessTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.user_other = User.objects.create_user(username="other", password="secret")
    ...
    @patch(
        "delft3dworker.models.Scenario.start",
        autospec=True,
    )
    ...
    def test_search(self):
        ...
        self.assertEqual(len(self._request(ScenarioViewSet, self.user_foo)), 1)

        self.assertEqual(len(self._request(SceneViewSet, self.user_foo)), 2)

        # User Bar can access only two own models (because Bar owns none)
        self.assertEqual(len(self._request(ScenarioViewSet, self.user_bar)), 0)
        self.assertEqual(len(self._request(SceneViewSet, self.user_bar)), 0)

        # User Foo and Other can see eachother, same company, in user list
        self.assertEqual(len(self._request(UserViewSet, self.user_foo)), 2)
        self.assertEqual(len(self._request(UserViewSet, self.user_other)), 2)

        # User Bar can only see themselves
        self.assertEqual(len(self._request(UserViewSet, self.user_bar)), 1)
...
class SceneTestCase(APITestCase):
    ...
    def test_multiple_scenes_publish_company(self, mocked_scene_method_company):
        ...
        response = self.client.post(
            url, {"suid": ["11111111-1111-1111-1111-111111111111"]}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.post(url, {"suid": ["something-rather-strange"]})
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(mocked_scene_method_company.call_count, 0)

        # view can handle wrong suids in data
        response = self.client.post(
            url, {"suid": ["00000000-0000-0000-0000-000000000000"]}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(mocked_scene_method_company.call_count, 0)
...
class SceneSearchTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.scene_1.info = {"postprocess_output": {"postproc_x": 0.1}}
        self.scene_1.save()
        ...
        self.scene_2.info = {
            "postprocess_output": {"postproc_x": 0.9, "postproc_y": 0.5}
        }
        self.scene_2.save()
...
class ScenarioTestCase(APITestCase):
    ...
    def test_scenario_start(self, mocked_scene_method):
        ...
        response = self.client.put(url, {}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        mocked_scene_method.assert_called_with(self.scenario, self.user_foo)
