...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with open("synchronize_data_sources.log", "a") as file:
            ...
            file.write('==================== Data Sync started -- {}\n'.format(str(datetime.datetime.now())))

            scenedirs = [scene.workingdir.encode('ascii','ignore') for scene in Scene.objects.all()]
            linked = set()
            ...
            self.stdout.ending = ''
            self.stdout.write('Checking for inconsistencies in the data storage components...');
            ...
            if len(unlinked) > 0:
                ...
                self.stdout.write('================================================================================');
                self.stdout.write('');
                self.stdout.write('## The following directories do not have any entries in the Postgres database:');
                self.stdout.write('');
                ...
                while not resp in ['y', 'n']:
                    self.stdout.write('## You can now enter a procedure which allows you to remove these directories.');
                    resp = raw_input('## Do you want to proceed? [y/N]: ').lower() or "n"
                ...
                if removedirs:
                    ...
                    for directory in unlinked:
                        ...
                        try:
                            if not rem == 'a':
                                rem = raw_input('{}: skip (default), delete, all? [S/d/a]: '.format(directory)).lower() or 's'

                            if rem in ['d', 'a']:
                                self.stdout.write("-- Removing {}... ".format(directory))
                                rmtree(directory)
            ...
            if len(unstored) > 0:
                ...
                self.stdout.write('================================================================================');
                self.stdout.write('');
                self.stdout.write('## The following simulations in the Postgres database do not have files stored');
                self.stdout.write('## on disk:');
                self.stdout.write('');
                for directory in unstored:
                    scene = Scene.objects.get(workingdir=directory + '/')
                    self.stdout.write('{} (current phase: {}, owner: {})'.format(scene, scene.get_phase_display(), scene.owner))
                ...
                if resetsims:
                    ...
                    for sim in unstored:
                        ...
                        try:
                            ...
                            rem = raw_input('{}: skip (default), reset, delete? [S/r/d]: '.format(scene)).lower() or 's'

                            if rem in ['r']:

                                if scene.phase == scene.phases.fin:
                                    self.stdout.write("-- Resetting Scene \"{}\"... ".format(scene))
                                    scene.reset()
                                    file.write('reset scene: {}\n'.format(scene))
                                    self.stdout.write("done\n")
                                else:
                                    self.stdout.write("-- Skipping \"{}\": scene not finished (current phase: {})\n".format(scene, scene.get_phase_display()))

                            elif rem in ['d']:

                                self.stdout.write("-- Deleting Scene \"{}\"... ".format(scene))
                                scene.delete()
            ...
            self.stdout.write('');
            self.stdout.write(' A log is written to file: synchronize_data_sources.log'.format(file));
            self.stdout.write('');
            self.stdout.write('################################################################################');
